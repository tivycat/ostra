...
document.querySelectorAll('input.autocomplete').forEach((el) => {
  el.addEventListener('input', (evt) => {
    let searchType = evt.target.dataset.searchType;
    let matchList = evt.target.nextElementSibling;

    search_with_autocomplete(evt.target.value, searchType, matchList)
  })
});
  ...
  if (searchType == 'students') {
    ...
    if (typeof data == 'object') {
      ...
      if (matches.length > 0) {
        // array of html strings
        const html = matches.map(match => `
      <div class="card card__body openCourse" data-teacher-name="${match.teacher_name}"data-teacher="${match.teacher_email}" data-course="${match.course_id}" data-course-title="${match.course_title}" data-students='${JSON.stringify(match.students)}'>
...
async function get_search_result_course(p) {
  ...
  clear_tbody().then(
    populate_object_tree(p, p.course)
  )
}
...
async function edit_user_open(p) {
  ...
  if (p.role === 'teacher') {
    ...
    Promise.all([
      get_teacher_doc(p.email), 
      get_coach_students(p.email)
    ]).then((result) => {
  ...
  if (p.role === 'student') {
    ...
    get_student_courses(p.email).then((querySnapshot) => {
...
function edit_user_save(role) {
  ...
  if (role === 'teacher') {
    ...
    if (OG_user.email === upd_user.email) {
      ...
      const delete_courses_from_prev_teacher_doc = ((courseList) => {
        return new Promise( (res) => {
          for (let c of courseList) {
            DB_TEACHERS.doc(c.teacher).get().then((doc) => {
              if (doc.exists) {
                let data = doc.data();
                ...
                doc.ref.update({ courses: filtered_data }).then(() => {
      ...
      const update_students = ((updatedUser) => {
        return new Promise((res) => {
          for (let course of updatedUser.courses) {
            DB.collectionGroup('courses').where('course_id', '==', course.course_id).get()
              .then((querySnapshot) => {
                querySnapshot.forEach((doc) => {
                  if (doc.exists) {
                    doc.ref.update({ teacher_email: course.teacher_email, teacher_name: course.teacher_name })
      ...
      Promise.all([
        update_teacher(upd_user),
        delete_courses_from_prev_teacher_doc(transfered_courses),
        update_students(upd_user)
    ...
    if (OG_user.email === upd_user.email) {
      ...
      let remove_courses = ((course_list) => {
        return new Promise((res) => {
          for (let c of course_list) {
            this_db.collection('courses').doc(c).delete().then(() => {
      ...
      let update_courses = ((course_list, doc_to_hardcopy) => {
        return new Promise((res) => {
          for (let c of course_list) {
            ...
            this_db.collection('courses').doc(c.course_id).set(
              hardCopy, { merge: true })
              .catch(e => console.error(e))
          }
          res()
      ...
      Promise.all([
        remove_courses(removed_courses),
        update_base_doc(base_doc),
        update_courses(upd_user.courses, base_doc)
      ]).then(() => {
    ...
    } else { // Om ny epost behövs så måste allt kopieras och läggas på en ny användare. Tidigare användare tas bort

      // Måste hämta kurserna för att hämta ALL data i dem (inte bara course_id etc som finns i localStorage)
      DB_STUDENTS.doc(OG_user.email).collection('courses')
        .get().then((querySnapshot) => {
          querySnapshot.forEach((doc) => {
            if (doc.exists) {
              ...
              DB_STUDENTS.doc(upd_user.email).collection('courses').doc(doc.id)
          ...
          for (let c of upd_user.courses) {
            DB_STUDENTS.doc(upd_user.email).collection('courses').doc(c.course_id)
              .set({
                class: upd_user.class,
                name: upd_user.name,
                coach_name: upd_user.coach_name,
          ...
          DB_STUDENTS.doc(OG_user.email).collection('courses').get()
            .then((querySnapshot) => {
              querySnapshot.forEach((doc) => {
                if (doc.exists) {
          ...
          DB_STUDENTS.doc(OG_user.email).delete().then(() => {
...
function create_new_teacher(p) {
  // Skapa lärare
  DB.collection('status-report').doc(CURRENT_TERM).collection('teachers').doc(p.email).set(
    {
      name: p.name,
      email: p.email,
      courses: []
...
function create_new_student(p) {
  ...
  let create_course = ((o) => {
    return new Promise((res) => {
      db.doc(o.email).collection('courses').doc(o.course_id)
        .set(o).then(res())
...
function update_coach_for_student(stu, coach) {
  ...
  let update_doc_base = (stu_email, c) => {
    return new Promise((res, rej) => {
      DB.collection('status-report').doc(CURRENT_TERM).collection('students').doc(stu_email).update({
        coach_email: c.email,
        coach_name: c.name
      }).then(() => {
  ...
  let update_each_course = (stu_email, c) => {
    return new Promise((res, rej) => {
      DB.collectionGroup('courses').where('email', '==', stu_email).get()
        .then((querySnapshot) => {
          querySnapshot.forEach((doc) => {
            if (doc.exists) {
              doc.ref.update({ coach_email: c.email, coach_name: c.name })
                .catch(e => { console.log(e) })
            }
          })
        }).then(() => {
  ...
  Promise.all([update_doc_base(stu, coach), update_each_course(stu, coach)])
    .then(() => {
...
function click_change_teacher_for_course(p) {
  ...
  // Ändra för lärare
  DB.collection('status-report').doc(CURRENT_TERM).collection('teachers').doc(p.email)
    .get().then((doc) => {
      if (doc.exists) {
  ...
  DB.collection('status-report').doc(CURRENT_TERM).collection('teachers').doc(p.original_teacher)
    .get().then((doc) => {
      if (doc.exists) {
        let data = doc.data();
